...
class MomentumBuyStrategy(BaseStrategy):
    def __init__(
        ...
        self.instrument_token = (
            256265  # Token for Nifty 50. Ideally fetch this dynamically.
        )

        self.state = "IDLE"  # IDLE, IN_TRADE, EXITED
        self.current_trade = None  # {order_id, symbol, entry_price, sl_price, type}
        self.last_candle_time = None
        self.last_check_minute = (
            -1
        )  # Track minute to avoid multiple checks in same minute boundary
    ...
    def check_entry(self):
        ...
        if not data or len(data) < 16:
            logger.warning(
                f"Insufficient data for Momentum check. Need 16 candles (15 for ATR + 1 to trade on), got {len(data) if data else 0}"
            )
            return
    ...
    def enter_trade(self, trade_type, spot_price, sl_level):
        ...
        logger.info(
            f"Momentum Signal! {trade_type} {symbol}. Spot: {spot_price}, SL Level: {sl_level}"
        )
    ...
    def manage_trade(self):
        ...
        # Log Strategy MTM
        logger.info(
            f"Strategy MTM: Momentum Buy ({symbol}): {current_mtm:.2f} | LTP: {ltp}"
        )

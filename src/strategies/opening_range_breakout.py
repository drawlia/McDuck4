...
class OpeningRangeBreakoutStrategy(BaseStrategy):
        ...
        try:
            ...
            if "ORBStrategy" in open_trades and open_trades["ORBStrategy"]:
                ...
                self.current_trade = {
                    ...
                    "order_id": "RESTORED",
                    "spot_entry_price": trade_data["entry_price"],
    ...
    def on_tick(self):
        ...
        if self.end_time and now >= self.end_time:
            if self.state == "IN_TRADE":
                ...
                self.state = "EXITED"
                logger.info(
                    f"End time {self.end_time} reached. Stopping ORB strategy for the day."
                )
            return
    ...
    def check_entry(self, current_dt):
        ...
        orb_candles_count = (
            self.orb_minutes // 5
        )  # Assuming 5min candles for calculation if interval is 5min
        if self.interval == "15minute":
        ...
        volma_t = (
            sum([c["volume"] for c in vol_bars]) / len(vol_bars) if vol_bars else 1
        )

        # 4. Calculate ATR(14)
        atr_candles = data[-16:-2]
        atr14 = (
            sum([(c["high"] - c["low"]) for c in atr_candles]) / 14
            if len(atr_candles) > 0
            else 50
        )
        ...
        logger.info(
            f"ORB Check [{candle_time.time()}]: C={close_t}, ORB_H={self.orb_high}, ORB_L={self.orb_low}, VWAP={vwap_t:.2f}, VolMA(5)={volma_t:.2f}, Vol={volume_t}, ATR14={atr14:.2f}"
        )

        # Signal Logic
        long_break = (
            (close_t > self.orb_high)
            and (close_t > vwap_t)
            and (volume_t > self.volume_mult * volma_t)
        )
        short_break = (
            (close_t < self.orb_low)
            and (close_t < vwap_t)
            and (volume_t > self.volume_mult * volma_t)
        )
    ...
    def enter_trade(self, trade_type, spot_price, atr14):
        ...
        # Get entry Option Premium for profit calculation BEFORE placing order
        entry_premium = 0
        ...
        if order_id:
            # Get the actual filled entry price from the latest trade/order
            # For now, we'll use the quote as the best estimate
            # In a real system, you'd query the order status to get the filled price
            actual_entry_quote = self.kite_client.get_quote([f"NFO:{option_symbol}"])
    ...
    def manage_trade(self):
        ...
        quote_key = (
            self.base_symbol if ":" in self.base_symbol else f"NFO:{self.base_symbol}"
        )
        ...
        if f"NFO:{option_symbol}" in quote:
            ...
            # If entry_premium is not set (None), capture it now
            if entry_premium is None or entry_premium <= 0:
                entry_premium = option_ltp
                self.current_trade["entry_premium"] = entry_premium
                logger.info(f"Entry premium captured on first tick: {entry_premium}")

            current_mtm = (option_ltp - entry_premium) * quantity

            # Log Strategy MTM
            logger.info(
                f"Strategy MTM: ORB ({option_symbol}): {current_mtm:.2f} (Entry: {entry_premium}, Current: {option_ltp}, Qty: {quantity}) | Target: {self.profit_target}"
            )

            if current_mtm >= self.profit_target:
                logger.info(
                    f"Profit Target Hit for ORB! {option_symbol} MTM: {current_mtm:.2f} >= {self.profit_target}. Exiting..."
                )
                self.exit_trade()
                return
